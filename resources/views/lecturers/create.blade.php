<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-lg font-semibold text-edu-text">Tambah Dosen</h1>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('lecturers.store') }}" class="edu-card border border-edu-hairline space-y-5">
            @csrf

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <x-input-label for="nidn" value="NIDN" />
                    <x-text-input id="nidn" name="nidn" :value="old('nidn')" required autofocus />
