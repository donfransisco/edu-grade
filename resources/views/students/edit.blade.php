<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-lg font-semibold text-edu-text">Edit Mahasiswa</h1>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('students.update', $student) }}" enctype="multipart/form-data" class="edu-card border border-edu-hairline space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <x-text-input id="nim" name="nim" :value="old('nim', $student->nim)" required />
                    <x-input-error :messages="$errors->get('nim')" />
                </div>
                <div>
                    <x-input-label for="nama" value="Nama Lengkap" />
                    <x-text-input id="nama" name="nama" :value="old('nama', $student->nama)" required />
                ...
                <div>
                    ...
                    <select id="angkatan" name="angkatan" class="edu-input" required>
                        ...
                        @for ($year = date('Y'); $year >= 2018; $year--)
                            <option value="{{ $year }}" {{ old('angkatan', $student->angkatan) == $year ? 'selected' : '' }}>{{ $year }}</option>
                        @endfor
                    </select>
            ...
            <div>
                ...
                <select id="program_studi" name="program_studi" class="edu-input" required>
                    ...
                    @foreach (\App\Models\Student::programStudiOptions() as $prodi)
                        <option value="{{ $prodi }}" {{ old('program_studi', $student->program_studi) === $prodi ? 'selected' : '' }}>{{ $prodi }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <x-text-input id="email" name="email" type="email" :value="old('email', $student->email)" required />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <x-text-input id="telepon" name="telepon" :value="old('telepon', $student->telepon)" />
                ...
                <div>
                    ...
                    @if ($student->foto)
                        <div class="mb-2">
                            <img src="{{ Storage::url($student->foto) }}" alt="{{ $student->nama }}" class="w-16 h-16 rounded-full object-cover border border-edu-hairline" />
                        </div>
                    @endif
            ...
            <div>
                ...
                <textarea id="alamat" name="alamat" rows="3" class="edu-input">{{ old('alamat', $student->alamat) }}</textarea>
