<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-lg font-semibold text-edu-text">Edit Mata Kuliah</h1>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('courses.update', $course) }}" class="edu-card border border-edu-hairline space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <x-text-input id="kode" name="kode" :value="old('kode', $course->kode)" required />
                    <x-input-error :messages="$errors->get('kode')" />
                </div>
                <div>
                    <x-input-label for="nama" value="Nama Mata Kuliah" />
                    <x-text-input id="nama" name="nama" :value="old('nama', $course->nama)" required />
            ...
            <div>
                ...
                <select id="lecturer_id" name="lecturer_id" class="edu-input" required>
                    ...
                    @foreach ($lecturers as $lecturer)
                        <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $course->lecturer_id) == $lecturer->id ? 'selected' : '' }}>{{ $lecturer->nama }} ({{ $lecturer->nidn }})</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <select id="sks" name="sks" class="edu-input" required>
                        ...
                        @for ($i = 1; $i <= 6; $i++)
                            <option value="{{ $i }}" {{ old('sks', $course->sks) == $i ? 'selected' : '' }}>{{ $i }}</option>
                        @endfor
                    </select>
